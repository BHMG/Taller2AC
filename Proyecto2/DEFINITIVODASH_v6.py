...
def callback_test_rapido(n_clicks,cor,day,prevqua,nac,dis,deb,tui,g,age,inter,pass1,pass2):
    if not n_clicks:
        ...
        probability1 = round(result.values[0], 2)
        probability2 = round(result.values[1], 2)
        probability3 = round(result.values[2], 2)

        return html.Div([
            f'   La probabilidad de que usted deserte es de {probability1*100}%, La probabilidad de que usted se gradue es de {probability2*100}%, La probabilidad de que usted simplemente se mantenga en su pregrado es de {probability3*100}% ',
            html.Br(),])
